...
@margin-note{在《Racket参考》中“（quote）”部分也有关于@racket[quote]的文档。}

@;{The @racket[quote] form produces a constant:}
@racket[quote]表产生一个常数：
...
@racket[datum]的语法在技术上被指定为@racket[read]函数解析为一个单个元素的任何内容。@racket[quote]表的值是相同的值，其@racket[read]将产生给定的@racket[_datum]。
...
another such value.}
@racket[_datum]可以是一个符号、一个布尔值、一个数值、一个（字符或字节）字符串、一个字符、一个关键字、一个空列表、一个包含更多类似值的点对（或列表），一个包含更多类似值的向量，一个包含更多类似值的哈希表，或者一个包含其它类似值的格子。
...
正如上面最后的示例所示，@racket[_datum]不需要匹配一个值的格式化的打印表。一个@racket[_datum]不能作为一个以@litchar{#<}开始的打印呈现，所以不能是@|void-const|、@|undefined-const|或一个过程。
...
@racket[quote]表很少用于一个@racket[_datum]，它是一个布尔值、数字或字符串本身，因为这些值的打印表已经可以用作常量。@racket[quote]表更常用于符号和列表，当没有被引用时，它具有其它含义（标识符、函数调用等等）。
...
这个简写几乎总是用来代替@racket[quote]。这个简写甚至应用于@racket[_datum]中，因此它可以生成一个包含@racket[quote]的列表。

@;{@refdetails["parse-quote"]{the @racketvalfont{@literal{'}} shorthand}}
@margin-note{在《Racket参考》里的“（parse-quote）”提供有更多关于@racketvalfont{@literal{'}}简写的内容。}
