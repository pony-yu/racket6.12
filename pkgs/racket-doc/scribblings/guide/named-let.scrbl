...
一个命名@racket[let]是一个迭代和递归表。它使用与局部绑定相同的语法关键字@racket[let]，但在@racket[let]之后的一个标识符（而不是一个最近的开括号）触发一个不同的解析。
...
也就是说，一个命名@racket[let]绑定一个只在函数主体中可见的函数标识符，并且用一些初始表达式的值隐式调用函数。
